...
namespace lab0
{
    internal class dbNormalizer
    {
        ...
        private readonly string __connStrSql = new SqliteConnectionStringBuilder
        {
            DataSource = "C:\\Users\\Admin\\Documents\\ПГНИУ\\ТРРП\\lab0\\lab0\\db.db"
        }.ConnectionString;
        ...
        private void insertRow(dbItem i)
        {
        ...
        private List<dbItem> readFromDb()
        {
            List<dbItem> items = new List<dbItem>();

            using(var conn=new SqliteConnection(__connStrSql))
            {
                conn.Open();
