...
COPY public.client (id, name, email, phone) FROM stdin;
\.
...
COPY public.item (id, name, category_id) FROM stdin;
\.
...
COPY public.main_table (id, shop_id, client_id, item_id, distr_id, amount, price, purchase_date) FROM stdin;
\.
...
COPY public.shop (id, name, address) FROM stdin;
\.
